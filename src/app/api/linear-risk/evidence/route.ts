...
const evidenceHandler = async (request: Request) => {
  try {
    ...
    const filename = `riesgo-lineal-${dateStamp}-${safeRisk}-${safeControl}-${hash}-evidencia${ext}`;
    ...
    console.error('Error saving evidence linear-risk:', error);
    return NextResponse.json({ error: 'Failed to save evidence' }, { status: 500 });
  }
};

export const POST = nextHandler(withModuleAccess('linear-risk', 'risk.linear.run', evidenceHandler));
