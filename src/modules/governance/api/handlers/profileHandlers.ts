...
import {
  ApproveProfileUseCase,
  CreateProfileUseCase,
  CreateSnapshotUseCase,
  GetActiveProfileUseCase,
} from '@/modules/governance/application/use-cases';
import { PrismaProfileRepository } from '@/modules/governance/infrastructure/repositories';
...
export async function getGovernanceActiveProfileHandler(request: Request) {
  try {
    ...
    const useCase = new GetActiveProfileUseCase(new PrismaProfileRepository());
    const profile = await useCase.execute(companyId);
    return NextResponse.json(profile);
  } catch (error: any) {
    console.error('Error fetching active governance profile:', error);
    return NextResponse.json({ error: 'Failed to fetch active profile' }, { status: 500 });
  }
}

export async function postGovernanceCreateProfileHandler(request: Request) {
  try {
    const body = (await request.json()) as { companyId: string; name: string; createdBy: string };
    if (!body?.companyId || !body?.name || !body?.createdBy) {
      return NextResponse.json({ error: 'Missing companyId, name or createdBy' }, { status: 400 });
    }

    const useCase = new CreateProfileUseCase(new PrismaProfileRepository());
    const profile = await useCase.execute(body);
    return NextResponse.json(profile);
  } catch (error: any) {
...
export async function postGovernanceApproveProfileHandler(request: Request) {
  try {
    const body = (await request.json()) as { profileId: string; approvedBy: string };
    if (!body?.profileId || !body?.approvedBy) {
      return NextResponse.json({ error: 'Missing profileId or approvedBy' }, { status: 400 });
    }

    const useCase = new ApproveProfileUseCase(new PrismaProfileRepository());
    const profile = await useCase.execute(body.profileId, body.approvedBy);
    if (!profile) {
    ...
    console.error('Error approving governance profile:', error);
    return NextResponse.json({ error: 'Failed to approve profile' }, { status: 500 });
  }
}

export async function postGovernanceCreateSnapshotHandler(request: Request) {
  try {
    const body = (await request.json()) as { profileId: string; runId?: string | null };
    if (!body?.profileId) {
      return NextResponse.json({ error: 'Missing profileId' }, { status: 400 });
    }

    const useCase = new CreateSnapshotUseCase(new PrismaProfileRepository());
    const snapshot = await useCase.execute({ profileId: body.profileId, runId: body.runId ?? null });
    return NextResponse.json(snapshot);
  } catch (error: any) {
    console.error('Error creating governance snapshot:', error);
    return NextResponse.json({ error: 'Failed to create snapshot' }, { status: 500 });
  }
}

export async function getActiveParamsProfileHandler() {
  try {
...
export async function postCreateParamsSnapshotHandler(request: Request) {
  try {
...
export async function postCreateParamsVersionHandler(request: Request) {
  try {
    ...
    const definitions = await prisma.$queryRaw<ParameterDefinition[]>(Prisma.sql`
      SELECT
        id,
        code,
        data_type_code,
